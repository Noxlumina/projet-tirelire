...
namespace TireLire.Controllers
{
    ...
    {
        //Instanciation du EFRepository pour l'entité Profile
        Repository<Client> repProfile= new EFRepository<Client>();
        ...
        public ActionResult ViewProfile(int id)
        {
            ...
            return View(repProfile.Trouver(id));
        }
        ...
        private void AssocierClient(string email)
        {
            ...
            { 
                ...
                Session["Client"] = RepClient.Lister().Where(c => c.Email == email).Where(c => c.Statut ==1).First();
            }
            catch (Exception)
            {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase fichier)
        {
            if (ModelState.IsValid)
            {
                ...
                if (fichier != null)
                {
                    model.UserPhoto = Path.GetFileName(fichier.FileName);

                }

                //RegisterViewModel nouveau = RepClient.Ajouter(model);
